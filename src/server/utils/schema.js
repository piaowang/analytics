import plywood_1 from 'sugo-plywood'
...
export function addExternal (source, external, mysqlTypes) {
  // 已经存在此表，重新添加前去掉旧数据
  if (_.some(tablesData, {TABLE_NAME: source})) {
    tablesData = tablesData.filter(c => c.TABLE_NAME !== source)
  }
  if (_.some(columnsData, {TABLE_NAME: source})) {
    columnsData = columnsData.filter(c => c.TABLE_NAME !== source)
  }
