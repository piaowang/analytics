...
async function queryOne(where) {
  return await db.SugoBehaviorAnalyticModels.findOne({
    where
  })
}

async function queryMany(where, other) {
  return await db.SugoBehaviorAnalyticModels.findAll({
    where,
    ...other
  })
}
...
async function create(model) {
  ...
  return await db.SugoBehaviorAnalyticModels.create({...model, name})
}
...
async function updateById(modelId, company_id, model) {
  if ('name' in model) {
    ...
    let existedSameNameModel = await db.SugoBehaviorAnalyticModels.findOne({
      where: {
        name,
        company_id
      }
  ...
  return await db.SugoBehaviorAnalyticModels.update(model, {
    where: {
      id: modelId,
      company_id
    }
...
async function deleteById(modelId, company_id) {
  return await db.SugoBehaviorAnalyticModels.destroy({
    where: {
      id: modelId,
      company_id
    }
