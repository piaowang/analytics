...
class LiveScreenWebsocketService extends SocketServiceBase {
  constructor() {
    super()
    this.ws = null
  ...
  async getData(queryObj, tag) {
    ...
    if(!data) {
      ...
      //有带过来的是新逻辑 可以前端设置 没带过来是旧逻辑 流量分析处的逻辑
      await redisSetExpire(tag, queryObj.intervalTime || (intervalTime / 2000), data)
    }
    return data
  }
...
const sugoLivescreenSocketQuerydruid = new LiveScreenWebsocketService()

sugoLivescreenSocketQuerydruid.on(events.query, function(id, fetcherDict) {
  let tags = []
  let querys = []
...
sugoLivescreenSocketQuerydruid.on(events.unmount, function(id, key) {
  this.clearByKey(id, key)
})

export default sugoLivescreenSocketQuerydruid
