...
const OTHER_API_DIRS =
  readdirSync(resolve(__dirname))
    .map(f => {
      const path = resolve(__dirname, f)
      const isDirectory = fs.statSync(path).isDirectory()
      if (isDirectory === true && f !== 'console-pages') {
        // 排除console-page目录配置（因为已经在console.page.js包含了该目录配置的路由）
        return path
      }
    })
    .filter(_.identity) || []
...
function checkDup(permissions) {
  let ids = _.uniq(permissions.map(i => i.id))

  if (permissions.length !== ids.length) {
    let errs = []
    for (let id of ids) {
      let count = permissions.filter(p => p.id === id)
      if (count.length > 1) {
        errs.push(id)
      }
    }
    if (errs.length) {
      exit('权限列表有路由有重复，请检查:' + errs.join(','))
    }
  }
}
