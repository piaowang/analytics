...
import { isEqualWithReactObj } from '~/src/common/sugo-utils'
...
export default class HorizontalSplitHelper extends React.Component {
  ...
  componentDidMount() {
    let {defaultFold } = this.props
    if(defaultFold) {
      if(defaultFold === 'left') {
        this.setState({ forceWeight0: 0 })
      } else if(defaultFold === 'right') {
  ...
  componentWillReceiveProps(nextProps) {
    if (!!isEqualWithReactObj(nextProps.children, this.props.children)) {
