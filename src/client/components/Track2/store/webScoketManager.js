...
import { APP_TYPE, iOS_RENDERER, getPageCorssConfigByPath } from '../constants'
import {
...
import { Action as HeatMapAction } from './heat-map-view-model'
...
const snapshotResponseHandle = async (payload, store) => {
  let { isHeatMap = false } = store.state.vm
  let newPayload = _.cloneDeep(payload)
  const VmAction = isHeatMap ? HeatMapAction : BuriedAction
  // store.dispatchAsyn({
  ...
  if (changeState) {
    ...
    if (oldPageInfo.page !== currPage) {
      const pageCorssConfig = getPageCorssConfigByPath(pageMap, currPage)
      let pageInfo = _.get(pageMap, [currPage])
      let pageData = {
        page: currPage,
        isH5: !!changeState.currentUrl,
        page_name: _.get(pageInfo, 'page_name', ''),
        category: _.get(pageInfo, 'category', ''),
        code: _.get(pageInfo, 'code', ''),
        defaultPageName: changeState.defaultPageName,
        xwalkScreenshots: changeState.xwalkScreenshots,
        is_submit_point: _.get(pageInfo, 'is_submit_point', false),
        ...pageCorssConfig
      }
...
const getTrackMessageResponseHandle = (payload, store) => {
  let { testMessage = [], isHeatMap = false } = store.state.vm
  const VmAction = isHeatMap ? HeatMapAction : BuriedAction
  ...
  setTimeout(() => {
    store.WsClient.send(JSON.stringify({ type: 'track_message_request' }))
  }, sdkGetSnapshotInterval * (isHeatMap ? 3 : 1))
  store.dispatchAsyn({
    type: VmAction.change,
    payload: { testMessage }
  })
...
const getDeviceInfoResponseHandle = (payload, store) => {
  ...
  const VmAction = store.state.vm.isHeatMap ? HeatMapAction : BuriedAction
  ...
  //获取页面信息和分类信息
  // if(!store.state.vm.isHeatMap) {
  store.dispatchAsyn({ type: VmAction.getPageInfo })
  // }
...
const handleStateIosStandard = function (deviceInfo, snapshot, state) {
  ...
  return {
    iosContent: content,
    classAttr: _.get(snapshot, 'serialized_objects.classAttr'),
    currentActivity,
    img,
    snapshot,
    h5ControlClass,
    ...
    defaultPageName: lastView.isH5 ? lastView.pageName : '',
...
const handleStateAndroid = function (snapshot, state) {
  ...
  return {
    classAttr: _.get(snapshot, 'classAttr'),
    viewMap,
    currentActivity,
    img,
    appMultiViews,
