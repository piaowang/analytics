...
class UploadModal extends React.Component{
  ...
  fetchOriginFile = async () => {
    ...
    const url = `/app/new-task-schedule/manager?project=${name}&ajax=listDepFile`
  ...
  deleteFile = async item => {
    ...
    const url = `/app/new-task-schedule/manager?project=${name}&ajax=delDepFile&fileName=${item}`
    let resp = await Fetch.get(url)
  ...
  handleUpload = async () => {
    ...
    let request = fileList.map(item => {
      ...
      return Fetch.post('/app/new-task-schedule/manager', {}, {
        body: formData,
        headers: {}
      })
    })
  ...
  render() {
    ...
    const props = {
      ...
      action: '/app/new-task-schedule/manager',
      multiple: true,
      onRemove: (file) => {
