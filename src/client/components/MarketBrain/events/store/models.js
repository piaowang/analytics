...
import * as modelsService from 'client/services/market-brain/models'

/**
 * 营销模型saga-model
 */
export const namespace = 'marketBrainModels'
...
export default {
  ...
  sagas: {
    ...
    *remove({ payload: id }, { call, put }) {
    ...
    /** 更新营销模型排序 */
    *updateOrders({ payload: { type, item } }, { call, put, select })  {
      const list = yield select(state => state.marketBrainModels.models)
      const models = _.clone(list)
      const size = _.size(models)
      const idx = _.findIndex(models, m => m.id === item.id)
  ...
  subscriptions: {
    init({ dispatch, history }) {
      ...
      if (pathname.includes('/console/market-brain-models')) {
