...
export default class MarketBrainResult extends Component {
  ...
  fetcher() {
    ...
    return (
      <AsyncTaskRunner 
        ...
        task={async (baseInfo) => {
          if (_.isEmpty(baseInfo)) return
          const { created_at, updated_at, url, params } = baseInfo
          const { 
            spreadEffectProject,
            spreadLinkField,
            spreadStaffIdField, 
        ...
        onResult={result => {
          if (!result) return
          const { spread = {}, deal = {}, staffGranularity = {} } = result
  ...
  renderTable() {
    ...
    const { spread, deal } = this.state

    const { actual_total, predict_total } = baseInfo
      ...
      <div className="bg-white pd3b pd3x height-80">
        <div className="base-info pd2y">
          <div style={{ 
            ...
            <div className='mg2l'>
              发送渠道: {['短信','电话', '微信'][baseInfo.send_channel]}
            </div>
          </div>
        </div>
