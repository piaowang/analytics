...
import { Select, Tooltip, Popconfirm, InputNumber, Input } from 'antd';
// import DistinctCascadeFetcher from '../../Fetcher/distinct-cascade-fetcher'
// import DistinctSelect from '../../Datasource/distinct-input'
...
export default class DQFilterItemMeasure extends React.Component {
  ...
  renderDistinctValue = () => {
    ...
    let isArrayVal = filter.type === 'in' || filter.type === 'notin'
    if(isArrayVal){
      return (
        <Input value={filter.value[0]} onChange={(e) => {
          onPickFilterProp(index, parentIndex, e.target.value, 0)
        }} className="iblock width80 mg1l" style={{verticalAlign: 'top'}} placeholder=""
        />
      )
    } else{
      return (
        <Input value={filter.value} onChange={(e) => {
          onPickFilterProp(index, parentIndex, e.target.value)
        }} className="iblock width80 mg1l" style={{verticalAlign: 'top'}} placeholder=""
        />
      )
    }
