...
function GooFlow(bgDiv,property){
  ...
  this.$customItemDblClick = property.customItemDblClick
  ...
  if(property.haveHead){
    ...
    for(var x=0;x<property.headBtns.length;++x){
      tmp+='<a className="pointer" class=\'GooFlow_head_btn\'><i class=\'ico_'+property.headBtns[x]+'\'></i></a>'
    }
    tmp+='</div>'
    this.$head = $(tmp)
    ...
    /* custom function */
    this.onBtnOpenFile = null
    this.onBtnSetFlied = null
    ...
    if(property.headBtns)
      this.$head.on('click',{inthis:this},function(e){
        ...
        switch($(tar).attr('class')){
          ...
          case 'ico_file':  if(This.onBtnOpenFile!=null)	This.onBtnOpenFile();break
          case 'ico_flied':  if(This.onBtnSetFlied!=null)	This.onBtnSetFlied();break
  ...
  if(property.haveTool){
    ...
    this.$tool.append('<div style=\'margin-bottom:5px\'><span/><span/><span/><span/></div>'
	  +'<a className="pointer" type=\'cursor\' class=\'GooFlow_tool_btndown\' id=\''+this.$id+'_btn_cursor\'><i class=\'ico_cursor\'/></a>'
     +'<a className="pointer" type=\'direct\' class=\'GooFlow_tool_btn\' id=\''+this.$id+'_btn_direct\'><i class=\'ico_direct\'/></a>'
    )
    if(property.toolBtns&&property.toolBtns.length>0){
      tmp='<span/>'
      for(var i=0;i<property.toolBtns.length;++i){
        tmp+='<a className="pointer" type=\''+property.toolBtns[i]+'\' id=\''+this.$id+'_btn_'+property.toolBtns[i].split(' ')[0]+'\' class=\'GooFlow_tool_btn\'><i class=\'ico_'+property.toolBtns[i]+'\'/></a>'//加入自定义按钮"+property.toolBtns[i]+"
      }
      this.$tool.append(tmp)
    }
    //加入区域划分框工具开关按钮
    if(property.haveGroup)
      this.$tool.append('<span/><a className="pointer" type=\'group\' class=\'GooFlow_tool_btn\' id=\''+this.$id+'_btn_group\'><i class=\'ico_group\'/></a>')
    toolWidth=31
    this.$nowType='cursor'
    //绑定各个按钮的点击事件
  ...
  if(this.$editable){
      ...
      This.addNode('_node_' + new Date().getTime(),{name: This.$nodeRemark[type],left:X,top:Y,type:This.$nowType})
      This.$max++
	  })
	  //划线或改线时用的绑定
...
GooFlow.prototype={
  ...
  initWorkForNode:function(){
    ...
    this.$workArea.delegate('.GooFlow_item > .span','dblclick',{inthis:this},function(e){
      ...
      if(e.data.inthis.$customItemDblClick) {
        var focusData = e.data.inthis.$nodeData[e.data.inthis.$focus]
        e.data.inthis.$customItemDblClick( e.data.inthis.$focus, focusData)
      }
      // var oldTxt=this.innerHTML;
      // var This=e.data.inthis;
  ...
  exportData:function(){
    ...
    for(var k1 in ret.nodes){
      if(ret.nodes[k1]['type'] === 'end round') {
        ret.nodes[k1]['type'] = 'end'
      }
      if(!ret.nodes[k1].marked){
        delete ret.nodes[k1]['marked']
      }	
      if(ret.nodes[k1].alt !== undefined){
        delete ret.nodes[k1]['alt']
      }
    }
    for(var k2 in ret.lines){
      ...
      if(ret.lines[k2].alt !== undefined){
        delete ret.lines[k2]['alt']
      }
    }
    return ret
  },
