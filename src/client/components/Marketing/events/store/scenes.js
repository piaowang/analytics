...
import { MARKETING_EVENT_STATUS, MARKETING_SEND_CHANNEL } from 'common/constants'
 ...
 * 营销模型saga-model
 */
export const namespace = 'marketingScenes'
...
export default {
  ...
  sagas: {
    ...
    *save({ payload: { id, values } }, { call, select, put }) {
      const model_id = yield select(state => state.marketingScenes.selected_model_id)
      let res
      values.model_id = model_id
      let shuldUpdatedCounter = false
      if (id) {
        const list = yield select(state => state.marketingScenes.list)
      ...
      if (shuldUpdatedCounter) {
        ...
        yield put({
          type: 'marketingModels/changeSelectedCounter',
          payload: {
            type,
            model_id,
            propKey: 'scene_total'
      ...
      if (id) {
        const expandedRows = yield select(state => state.marketingScenes.expandedRows)
    ...
    *fetchEvents({ payload }, { call, put, select }) {
      const oldExpandedRows = yield select(state => state.marketingScenes.expandedRows)
      const res = yield call(eventsService.fetch, payload)
      let expandedRows = _.clone(oldExpandedRows)
      expandedRows[payload.scene_id].events = _.get(res, 'result.rows', [])
    ...
    * updateEvent({ payload }, { call, put, select }) {
      ...
      const status = event.status === MARKETING_EVENT_STATUS.OPEN ? 0 : 1
      const type = event.status === MARKETING_EVENT_STATUS.OPEN ? 'DECREMENT' : 'INCREMENT'
      // 更新事件状态
      ...
      yield put({
        ...
        payload: {
          expandedRows: {
            ...expandedRows,
            ...immutateUpdate(expandedRows, [event.scene_id, 'events', idx], o => ({ ...o, status }))
          }
        }
      })
    }
