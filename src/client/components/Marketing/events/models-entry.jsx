 ...
 */
@connect(state => ({
  ...state['marketingModels'],
  loading: state.loading.models['marketingModels']
}))
@Form.create()
...
export default class ModelsEntry extends Component {
  ...
  renderLeftList = withCommonFilter(commonFilter => {
    ...
    const modelMenus = _.isEmpty(models) ? (
      ...
      <Menu mode="inline" selectedKeys={selectedKey} onClick={this.modelClickHanlder}>
        {
          models.filter(item => searching ? smartSearch(searching, item.name) : true).map((item, idx) => {
            return (
              <Menu.Item model={item} key={item.id}>
                <div className="m-title">{item.name}</div>
                <div className="m-sub-content">
                  共有{item.scene_total}个启用场景，{item.event_total}个启用事件
                  <span className="mg3l opt-btns hide">
                    <Tooltip title="置顶">
                      <Icon type="sugo-stick" onClick={() => this.sortHandler(item, idx, 'moveTop')} className="mg1r pointer font16"/>
                    </Tooltip>
                    <Tooltip title="上移">
                      <Icon type="sugo-upward" onClick={() => this.sortHandler(item, idx, 'moveUp')} className="mg1r pointer font16"/>
                    </Tooltip>
                    <Tooltip title="下移">
                      <Icon type="sugo-downward" onClick={() => this.sortHandler(item, idx, 'moveDown')} className="mg1r pointer font16"/>
                    </Tooltip>
                    {Number(item.scene_total) === 0 && Number(item.event_total) === 0 ? (
                      <Icon type="sugo-delete" className="font16" onClick={(e) => this.deleteHandler(item.id, e)}/>
                      // 在这种布局下使用 Popconfirm 会导致鼠标无法移动到该元素上
  ...
  sortHandler = (item, idx, type) => {
    this.props.dispatch({
      type: 'marketingModels/updateOrders',
      payload: {
        type,
        item,
        idx
