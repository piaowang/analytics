...
export default function getNumber(styleConfig, updateFn) {
  ...
  return [
    ...
    new StyleItem({
      ...
      items: [
        new StyleItem({
          ...
          type: 'number',
          min: 0,
          value: _.get(styleConfig, '_extra.option.progressHeight', 6),
          onChange(v) {
            updateFn('_extra.option.progressHeight', () => v)
          }
        })
      ]
    }),
    ...
    new StyleItem({
      ...
      type: 'editorGroup',
      hidable: true,
      checked: _.get(styleConfig, '_extra.option.numberStyle.showInfo', true),
      onChangeVisible: function (e) {
        e.stopPropagation()
        const checked = e.target.checked
        updateFn('_extra.option.numberStyle.showInfo', () => checked ? true : false)
      },
      items: [
        new StyleItem(commonStyleItem.color, {
          name:'numberStyleColor',
    ...
    new StyleItem({
      ...
      type: 'editorGroup',
      items: [
        commonStyleItem.setWarning(styleConfig, updateFn)
      ]
    })
  ]
}
