...
import { APP_TYPE, BORDER_WIDTH } from '../constants'
import 'seedrandom'

//界面绑定事件和数据存储结构的转换
function dimBindingToDataBinds(dimBinding, type) {
  return _.reduce(dimBinding, (r, v) => {
    if (type === APP_TYPE.h5) {
      r[v.dimension] = _.pick(v, ['path', 'similar'])
      return r
    }
    r[v.dimension] = _.pick(v, ['sugo_autotrack_path', 'sugo_autotrack_position'])
    return r
    // if (_.has(v, 'sugo_autotrack_path')) {
    //   r[v.dimension] = _.pick(v, ['sugo_autotrack_path', 'sugo_autotrack_position'])
    //   return r
    // }
    // r[v.dimension] = v.path
    // return r
  }, {})
}
...
/**
 * 事件高级上报设置信息回绑
 * @param {*} event 数据库中的event信息
 * @param {*} type 事件类型
 */
function dataBindsToDimBinding(event, type) {
  if (type === APP_TYPE.h5 && !event.binds) {
    return []
  }

  if (type !== APP_TYPE.h5 && !event.code) {
    return []
  }

  const dataBinds = type === APP_TYPE.h5
    ? event.binds
    : JSON.parse(event.code || '{}')
  return _.reduce(dataBinds, (r, v, k) => {
    ...
    if (_.isString(v)) {
      r.push({ dimension: k, path: v })
      return r
    }
    r.push({ dimension: k, ...v })
    return r
  }, [])
}
 ...
 * @returns
 */
async function getScreenShot(imgUrl, newEvent, xwalkScreenshots = {}, controls = {}, imgHeight, imgWidth) {
  ...
  for (let key in xwalkScreenshots) {
    ...
    const view = controls.find(p => p.hashCode === key) || { position: {} }
    const xwalk_img = await new Promise((resolve) => {
      const _img = new Image()
      _img.onload = () => resolve(_img)
      _img.onerror = () => resolve(_img)
      _img.src = xwalkImgUrl
    })
    ctx.drawImage(xwalk_img, 0, 0, xwalk_img.width, xwalk_img.height, view.position.x, view.position.y, view.position.width, view.position.height)
  }

  let { width, height, top, left } = newEvent.position || {}
...
// 获取h5控件的样式map
function getPathClassMap(htmlNodes) {
