...
function getClickEventProp(appType, viewMap, viewHashCode, currentActivity, iosMainView, currentUrl) {
  ...
  if (appType === APP_TYPE.ios) {
    ...
    editEvent.page = currentActivity
    editEvent.sugo_autotrack_position = _.get(view, 'sugo_autotrack_position', '')
    editEvent.sugo_autotrack_content = _.get(view, 'sugo_autotrack_content', '')
    editEvent.sugo_autotrack_path = _.get(view, 'sugo_autotrack_path', '')
    editEvent.sugo_autotrack_page_path = _.get(view, 'sugo_autotrack_page_path', '')
    ...
    let webViewInfo = {}
    if (viewMap.androidViewMap) {
      webViewInfo = _.values(viewMap.androidViewMap).find(p => _.get(p, 'htmlPage.url') === currentUrl) || {}
    } else {
      const id = _.findKey(iosContent.cachedObjects, p => _.get(p, 'htmlPage.url') === currentUrl)
      webViewInfo = _.get(iosContent.cachedObjects, id, {})
    } 

    var id = { path }
    editEvent.viewHashCode = id
    editEvent.sugo_autotrack_position = _.get(webViewInfo, 'sugo_autotrack_position', '')
    editEvent.sugo_autotrack_path = path
    editEvent.sugo_autotrack_page_path = _.get(webViewInfo, 'sugo_autotrack_page_path', '')
    ...
    if (viewHashCode.innerText) {
      ...
      editEvent.sugo_autotrack_content = editEvent.event_name
    }
  }
  return editEvent
}
